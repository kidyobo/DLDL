...
public class T4MSC : EditorWindow {
	...
	string T4MEditorFolder = "Assets/Third/T4M/Editor/";
	string T4MFolder = "Assets/Third/T4M/";
    private string T4MPrefabFolder
    {
        get
        {
            var scene = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene();
            var scenePath = scene.path.Substring(0,scene.path.Length-6) + "/T4MOBJ/";
            return scenePath;
        }
    }
    string FinalExpName;
					...
					if (CurrentSelect.GetComponent <T4MObjSC>() && (!CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial || !CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMesh)){
						...
						if (rendererPart.Length==0){
							...
							for (int i=0;i<rendererPart.Length;i++){
								...
								EditorUtility.SetSelectedRenderState(rendererPart[i], EditorSelectedRenderState.Hidden);
							}
						}
					}
	 ...
	 void ConvertUTerrain()
	{
		...
		EditorUtility.SetSelectedRenderState(Child.GetComponent<Renderer>(), EditorSelectedRenderState.Hidden);
		...
		AssetDatabase.StartAssetEditing();
			...
			OBJI.globalScale = 1;
			OBJI.importTangents = ModelImporterTangents.Import;
			OBJI.importNormals =ModelImporterNormals.Import;
