...
namespace UnityEditor.XCodeEditor
{
    public partial class XCProject : System.IDisposable
    {
        ...
        private PBXProjectTest _project;
            ...
            : this()
        {
            ...
            {
                _project = new PBXProjectTest(_rootObjectKey, (PBXDictionary)_objects[_rootObjectKey]);
        ...
        #region Mods
        public void ApplyMod(string pbxmod, List<string> add)
        {
            XCMod mod = new XCMod(pbxmod);
            ApplyMod(mod, add);
        }

        public void ApplyMod(XCMod mod, List<string> needAdd)
        {
            ...
            foreach (string filePath in mod.files)
            {
                ...
                if (needAdd.Count > 0 && needAdd.IndexOf(fileName) >= 0)
                {
                    //说明已经add了
                    needAdd.Remove(fileName);
                }
                string absoluteFilePath = System.IO.Path.Combine(mod.path, filePath);
                this.AddFile(absoluteFilePath, modGroup);
            }
            foreach (string add in needAdd)
            {
                Debug.Log("添加没有配置的库:= " + add);
                string addName = "sdk/" + add;
                string absoluteFilePath = System.IO.Path.Combine(mod.path, addName);
                this.AddFile(absoluteFilePath, modGroup);
            }
