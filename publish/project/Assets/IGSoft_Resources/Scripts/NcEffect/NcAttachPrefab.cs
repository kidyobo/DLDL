...
public class NcAttachPrefab : NcEffectBehaviour
{
    ...
    public override int GetAnimationState()
    {
        ...
        {
            if (((m_nRepeatCount == 0 && m_nCreateCount < 1) ||
    ...
    public GameObject GetInstanceObject()
    {
        if (goList == null)
            UpdateImmediately();
        return ((goList == null || goList.Count < 1) ? null : goList[0]);
    }
    ...
    protected virtual void Awake()
    {
        ...
        if (IsCreatingEditObject() == true)
            m_bEnabled = false;
#endif
        goList = new List<GameObject>(Mathf.Max(1, m_nRepeatCount));
        if (m_AttachPrefab != null)
        {
            m_origionRot = m_AttachPrefab.transform.localRotation;
        }
        Update();
    }
    ...
    void CreateAttachPrefab()
    {
        ...
        if (0 == m_fRepeatTime && 0 < m_nRepeatCount && m_nCreateCount < m_nRepeatCount)
            CreateAttachPrefab();
    }
    ...
    void CreateAttachGameObject()
    {
        ...
        {
            createObj = (GameObject)CreateGameObject(GetTargetGameObject(), (GetTargetGameObject() == gameObject ? null : transform), m_AttachPrefab);
    ...
    public override void OnSetActiveRecursively(bool bActive)
    {
        ...
        for (int n = 0; n < goList.Count; n++)
            NsEffectManager.SetActiveRecursively(goList[n], bActive);
    }
