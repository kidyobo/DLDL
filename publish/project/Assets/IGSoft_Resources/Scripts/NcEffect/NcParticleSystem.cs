...
// 			return "SCRIPT_EMPTY_ATTACHPREFAB";
	...
	void Awake()
	{
        m_ps = this.GetComponent<ParticleSystem>();
    }
...
//				SetEnableParticle(false);
	...
	void SetEnableParticle(bool bEnable)
	{
        ...
        if (m_ps != null)
        {
            NsFixer.EnableEmission(m_ps, bEnable);
        }
	}
...
// 			m_ps.startDelay		+= m_fStartDelayTime;
            NsFixer.SetEmissionRateMul(m_ps, m_fStartEmissionRate);
            NsFixer.SetParticleMainMul(m_ps, m_fStartSizeRate, m_fStartLifeTimeRate, m_fStartSpeedRate);
//			iphone error!!!
...
//		m_fStartSpeedRate		= 1;
	...
	public void SaveShurikenSpeed()
	{
		...
		if (ps != null)
		{
			SerializedObject sysSo = new SerializedObject(ps);
