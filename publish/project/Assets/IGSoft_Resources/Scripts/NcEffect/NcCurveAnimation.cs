...
public class NcCurveAnimation : NcEffectAniBehaviour
{
	...
	public class NcInfoCurve
	{
		...
		public		enum APPLY_TYPE						{ NONE, POSITION, ROTATION, SCALE, MATERIAL_COLOR, TEXTUREUV };
		public		static string[]	m_TypeName			= { "None", "Position", "Rotation", "Scale", "MaterialColor", "TextureUV" };
	...
	public override string CheckProperty()
	{
		...
		foreach (NcInfoCurve curveInfo in m_CurveInfoList)
		{
			...
			if (curveInfo.m_ApplyType == NcInfoCurve.APPLY_TYPE.MATERIAL_COLOR)
				if (curveInfo.m_bRecursively == false && (r == null || Ng_GetMaterialColorName(r.sharedMaterial) == null))
				...
				case NcInfoCurve.APPLY_TYPE.MATERIAL_COLOR:
					{
						if (curveInfo.m_bRecursively)
						{
							...
							if (_renderer != null)
							{
								m_ColorName		= Ng_GetMaterialColorName(_renderer.sharedMaterial);
								if (m_ColorName != null)
								{
									if (m_bSavedOriginalValue == false)
										curveInfo.m_OriginalValue = _renderer.material.GetColor(m_ColorName);
									else _renderer.material.SetColor(m_ColorName, curveInfo.m_OriginalValue);
								}
				...
				case NcInfoCurve.APPLY_TYPE.MATERIAL_COLOR:
					{
						if (curveInfo.m_bRecursively)
						{
							...
							if (_renderer != null && m_ColorName != null)
							{
                                var m_MainMaterial=_renderer.material;
                                // this Only
