...
@interface SDKConnector()<UNUserNotificationCenterDelegate>
@end
...
extern "C"{
    ...
    void CallSDKFunc(const char*type,const char*jsonpara){
        ...
        [[SDKConnector sharedInterce] _CallSDKFunc:s_calltype:s_calljsonpara];
    };
...
static SDKConnector*connect;
//*********************************通用SDKConnector类**************************************************
@implementation SDKConnector

+(SDKConnector*)sharedInterce{
    if(connect==NULL){
        connect=[[SDKConnector alloc]init];
    }
    return connect;
}
...
-(void)_CallSDKFunc :(NSString*)type :(NSString*)jsonpara
{
    ...
    {//初始化
        NSLog(@"+++调用SDK初始化借口啦+++++++++++++++++");
        [[LeHaHaControl sharedInterce] initSDK];
    }
    ...
    {//充值
        NSLog(@"+++调用充值接口+++++++++++++++++");
        [[LeHaHaControl sharedInterce]pay:jsonDict];
    }
    else if ([type isEqualToString:@"reportData"])
    {//上报
        [[LeHaHaControl sharedInterce]reportData:jsonDict];
    }
    else if ([type isEqualToString:@"doLogout"])
    {//上报
        [[LeHaHaControl sharedInterce]logout];
    }
...
-(void)_CallBackToUnity:(NSDictionary *)dict
{
    ...
    UnitySendMessage("Root", "OcCallUnityBack", [param UTF8String]);
}
