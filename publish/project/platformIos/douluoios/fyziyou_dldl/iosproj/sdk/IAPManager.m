	...
	[self removeAllUncompleteTransactionsBeforeNewPurchase];
        if ([SKPaymentQueue canMakePayments]) { //用户允许app内购
            if (productId.length) {
                ...
                NSLog(@"商品为空");
                [[LeHaHaControl sharedInterce]payFailed:@"商品为空"];
                goodsRequestFinished = YES; //完成请求
            }
        } else { //没有权限
            [[LeHaHaControl sharedInterce]payFailed:@"没有权限"];
            goodsRequestFinished = YES; //完成请求
        }
    } else {
        NSLog(@"上次请求还未完成，请稍等");
         [[LeHaHaControl sharedInterce]payFailed:@"上次请求还未完成，请稍等"];
    }
}
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response {
    ...
    if([product count] == 0){
        ...
        NSLog(@"--------------没有商品------------------");
        [[LeHaHaControl sharedInterce]payFailed:@"反馈没有商品"];
        return;
    }
...
- (void)request:(SKRequest *)request didFailWithError:(NSError *)error {
    [[LeHaHaControl sharedInterce]payFailed:@"查询失败"];
    goodsRequestFinished = YES; //失败，请求完成
}
    ...
    for (SKPaymentTransaction *transaction in transactions) {
        switch (transaction.transactionState) {
            ...
            case SKPaymentTransactionStateFailed://交易失败
                [[LeHaHaControl sharedInterce]payFailed:@"交易失败"];
...
- (void)getReceipt: (SKPaymentTransaction *)transaction{
    ...
    [[LeHaHaControl sharedInterce] paySucess:receipt :payExt :orderId];
