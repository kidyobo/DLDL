 ...
 * 
 */
export class GetEquipGuider extends BaseGuider {
    private equipInfos: GetEquipInfo[] = [];

    constructor() {
        super(EnumGuide.GetEquip, 0, EnumGuiderQuestRule.PauseAbsolutely, false);
    }
    ...
    protected _initSteps(): void {
        this._addStep(EnumGuide.GetEquip_OpenView, this._onStepOpenView);
        this._addStep(EnumGuide.GuideCommon_None, null);
        this.m_activeFrame = EnumGuide.GetEquip_OpenView;
    }

    private _onStepOpenView(step: EnumGuide): void {
        // 打开面板
        G.ViewCacher.getEquipView.open();
    }
    ...
    protected _forceStep(step: EnumGuide): boolean {
        ...
        G.ViewCacher.getEquipView.close();
