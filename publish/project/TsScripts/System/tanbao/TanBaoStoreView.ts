...
export class TanBaoStoreView extends CommonForm {
    ...
    protected resPath(): string {
        return UIPathData.TanBaoStoreView;
    }
    protected onClose() {
    ...
    protected initElements(): void {
        ...
        this.allItemList.Count = TanBaoStoreView.MAX_CAPACITY;
    ...
    onTgbjStoreChange(type: number): void {
        if (!this.isOpened || type != Macros.CONTAINER_TYPE_SKYLOTTERY) {
            return;
        }
        // 刷新仓库
        let tgbjData: TgbjData = G.DataMgr.tgbjData;
        let rawDatas: { [pos: number]: ThingItemData } = G.DataMgr.thingData.getContainer(Macros.CONTAINER_TYPE_SKYLOTTERY);
        if (null == this.listData) {
            this.listData = new Array<TgbjStoreItemData>(TanBaoStoreView.MAX_CAPACITY);
            for (let i: number = 0; i < TanBaoStoreView.MAX_CAPACITY; i++) {
        ...
        for (let i = 0; i < TanBaoStoreView.MAX_CAPACITY; i++) {
        ...
        // 刷新容量      
        let colorText: string = TextFieldUtil.getColorText(G.DataMgr.thingData.getCurTgbjStorePosNum().toString(), Color.GREEN) + TextFieldUtil.getColorText('/' + Macros.MAX_SKYLOTTER_THINGOBJ_LIST_NUMBER, Color.BEIGE);
   ...
   */
    private showBagUI(item: ListItem): void {
        ...
        if (!item.data.iconItem) {
            ...
            iconItem.setTipFrom(TipFrom.tanbao);
    ...
    private _onClickBtnSort(): void {
        G.ModuleMgr.netModule.sendMsg(ProtocolUtil.getOperateContainerMsg(Macros.CONTAINER_OPERATE_SORT, Macros.CONTAINER_TYPE_SKYLOTTERY));
    }
    ...
    private _fetchAll(): void {
        ...
        let posNum: number = G.DataMgr.thingData.getCurTgbjStorePosNum();
        if (posNum <= 0) {
            return;
        }
        // 检查背包格子数量
        ...
        if (canFetchAll) {
            ...
            G.ModuleMgr.netModule.sendMsg(ProtocolUtil.getSwapContainerMsg(Macros.CONTAINER_TYPE_SKYLOTTERY, idList, posList, numList, Macros.CONTAINER_TYPE_ROLE_BAG, Macros.UNDEFINED_CONTAINER_POSITION));
        }
    }
